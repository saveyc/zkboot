...
//    u16 i;
    ...
    {
        recv_message_from_sever(&(tcp_client_list[0].tcp_recv_buf[0]), &(tcp_client_list[0].tcp_recv_len));
    ...
    else if(tcp_client_list[0].tcp_client_statue == CLIENT_CONNECT_OK )
    {
        ...
        if(tcp_client_list[0].tcp_send_en == 1)
        {
            ...
            if(Programm_Completed_Reply_flag == 1)//升级完成反馈应答标志
            {
                Programm_Completed_Reply_flag=0;
                Delay(5000);  //5s  //2s--old  //for(i=0;i<=2000;i++)
                NVIC_SystemReset();
            }                            
        }
    }
