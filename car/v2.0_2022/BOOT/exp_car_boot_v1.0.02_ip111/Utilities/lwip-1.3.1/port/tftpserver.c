...
uint32_t RamSource;
uint8_t  Programm_Completed_Success = 0;
                          ...
                          struct ip_addr *to_ip, u16_t to_port)
{
    ...
    //改成读FLASF数据 一次读512 Byte 小于512Byte 为结束
...
void wrq_recv_callback(void *_args, struct udp_pcb *upcb, struct pbuf *pkt_buf, struct ip_addr *addr, u16_t port)
{
            ...
            (tftp_extract_block(pkt_buf->payload) == (args->block + 1)))
    {
        ...
        //将接收到的数据写到FLASH
        RamSource = (uint32_t)pkt_buf->payload + TFTP_DATA_PKT_HDR_LEN;
        ...
        if(n < 512)
        {
            Programm_Completed_Success = 1;
            file_tot_bytes = (args->tot_bytes) + (pkt_buf->len - TFTP_DATA_PKT_HDR_LEN);
        }
        // if (n <= 0)
        //{
...
void process_tftp_request(struct pbuf *pkt_buf, struct ip_addr *addr, u16_t port)
{
    ...
    case TFTP_WRQ:    /* TFTP WRQ (write request)   客户端发TFTP数据请求，服务器接收数据命令*/
        if(g_download_status != DLOAD_PARA) return;
        ...
        if(sDLoad_Para_Data.mode == DLOAD_MODE_LOCAL)
            ...
            FlashDestination = AppTempAddress;
        else
            return;
        /* Erase the needed pages where the user application will be loaded */
        /* Define the number of page to be erased */
        NbrOfPage = FLASH_PagesMask(0x18000);//擦除96K

        /* Erase the FLASH pages */
        FLASH_Unlock();
        if(FlashDestination == ApplicationAddress)
        {
            FLASHStatus = FLASH_ErasePage(UserAppEnFlagAddress);
           for (EraseCounter = 0; (EraseCounter < NbrOfPage) && (FLASHStatus == FLASH_COMPLETE); EraseCounter++)
           {
               FLASHStatus = FLASH_ErasePage(FlashDestination + (PageSize * EraseCounter));
           }
        }
        else if(FlashDestination == AppTempAddress)
        {
//            FLASHStatus = FLASH_ErasePage(UserAppEnFlagAddress);
           for (EraseCounter = 0; (EraseCounter < NbrOfPage) && (FLASHStatus == FLASH_COMPLETE); EraseCounter++)
           {
