...
void   udp_client_process(void)
{
    ...
    {
        vpcfun_recv_message_from_sever(&(tcp_client_list[0].tcp_recev_buf[0]), &(tcp_client_list[0].tcp_recev_len));
...
{
#ifndef     USE_UDP
    if( ( tcp_client_list[0].tcp_client_statue == CLIENT_CONNECT_OK ) && ( tcp_client_list[1].echoclient_pcb->state == ESTABLISHED) )
    {
...
void get_ip_para_process(u8 tcp_offset)
{
    ...
    {
        tcp_client_list[tcp_offset].dest_port = 9001;
