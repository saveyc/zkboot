...
u8  user_para_driver_buff[200];
u16 user_para_dri_len;

u16 upload_user_para_buff[FLASH_DATA_LEN];
...
void  fun_read_user_driver_para_handle(void)
{
    u8 i = 0;
    u16 flag;
    u8 data;
    ...
    if (flag == 0xAA)
    {
        len = *((u16*)(UserParaStartAddress + 42));
        for (i = 0; i < len; i++) 
        {
            data = *((u16*)(UserParaStartAddress + 44 + 2 * i));
            user_para_driver_buff[i] = data;
        }
    }
    else 
    {
        for (i = 0; i < len; i++) 
        {
            user_para_driver_buff[i] = 0;
            len = 0;
        }
    }
    user_para_dri_len = len;
}

void fun_upload_user_para_handle(void)
{
    u16 i = 0;

    for (i = 0; i < FLASH_DATA_LEN; i++) {
        upload_user_para_buff[i] = *((u16*)(UserParaStartAddress + i * 2 + 2));//读出所有的flash参数
    }
}



void fun_write_user_para_handle(u8* point,u16 len)
{
    u16 i;
    if (para_type == BASIC_PARA)
    {
        for (i = 0; i < 20; i++)
        {
            upload_user_para_buff[i] = (point[2 * i] | (point[2 * i + 1] << 8));
        }
    }
    ...
    for (i = 0; i < FLASH_DATA_LEN; i++)
    {
        FLASH_ProgramHalfWord(UserParaStartAddress + 2 + 2 * i, upload_user_para_buff[i]);
    }
    FLASH_Lock();
}
...
void recv_msg_process(u8 *point)
{
    ...
    case RECV_MSG_WRITE_PARA_TYPE:
        para_type = BASIC_PARA;
        fun_upload_user_para_handle();
        fun_write_user_para_handle(point + sizeof(MSG_HEAD_DATA), head->MSG_LENGTH - 11);
...
void  reply_recv_read_dri_para_msg(u8* buf, u16* len, u16 type)
{
    u8 sum = (type & 0xFF) ^ (type >> 8);
...
void send_user_driver_para_msg(u8* buf, u16* len, u16 type)
{
    u8 sum;
    u16 sendlen;
    u16 i;

    sendlen = user_para_dri_len + 11;

    buf[9] = type & 0xFF;
    buf[10] = (type >> 8) & 0xFF;

    for (i = 0; i < user_para_dri_len; i++)
    {
        buf[11 + i] = user_para_driver_buff[i];
    }
    sum = buf[9];
    for (i = 1; i < sendlen - 9; i++)
    {
