...
void vcan_sendframe_process_one(void)
{
    ...
    if (Local_station == 1) {
        if ( (canTxMsg->extId.seg_polo == CAN_SEG_POLO_NONE) || (canTxMsg->extId.seg_polo == CAN_SEG_POLO_FIRST) || (canTxMsg->extId.seg_polo == CAN_SEG_POLO_FINAL)) {
...
void master_can_bus_frame_receive(CanRxMsg rxMsg)
{
    ...
    if (recv_cmd == DLOAD_CMD_ENTER_BOOT && g_download_status == DLOAD_TRAN_INIT)
    {
          ...
          if( (rxMsg.Data[0] == 0x55) && (rxMsg.Data[1] == 0x55)){
